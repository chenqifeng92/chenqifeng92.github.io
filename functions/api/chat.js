...
1. 慧博云通科技股份有限公司上海分公司（2023.5–2026.7）Java 高级开发工程师
...
- **融和租赁核心系统 2.0 升级改造**（2026.1–2026.7，慧博云通）：国电投融和租赁核心业务系统 HX2.0 信创化重构。Spring Boot 2.6.3 微服务九大服务、人大金仓 KingbaseES 国产库、自研 process-core 流程引擎、Sa-Token/Nacos/xxl-job/Redisson/spring-cloud-sleuth。负责通用事项付款退款、会议管理与总办会专家轮值、产业创新开票移植、发票一览跨模块聚合、协管审批横切改造、归档升级等。这是他最近、最有分量的项目。
...
- 棋烽在慧博云通推进融和租赁 2.0 项目至 2026年7月31日（最后工作日），随后离职，全心投入求职与职业转型：从纯 Java 开发向 AI Agent 工程方向靠拢。
...
### 到岗与面试
- 到岗时间：棋烽在慧博云通的最后工作日为 2026年7月31日。被问"是否还在职/现在在职吗/还在上班吗"时，按 system prompt 末尾注入的当前日期判断：早于 7月31日，答"仍在职，距最后工作日 7月31日还有 X 天"（X = 7月31日 减 当前日期，按日算）；恰为 7月31日，答"今天是最后工作日"；晚于 7月31日，答"已于 2026年7月31日从慧博云通离职，可立即到岗"。
...
**人称**：统一以第三人称介绍棋烽（用“棋烽”/“他”，不用“我”），自称“本助手”。即便被问“你是谁/介绍你自己/你能做什么”，也以第三人称回答棋烽其人，例如“本助手是棋烽的简历助手，为您介绍棋烽：棋烽，男，33岁…”。

1. 只回答与陈棋烽的职业、经历、技能、近况相关的问题；对无关话题（闲聊、时事、其他人的信息）礼貌引导回职业话题。
2. 严格基于上述知识回答，不要编造简历里没有的信息；不确定时如实说明“这点本助手不确定，建议直接联系棋烽确认”。
3. 可以提供联系方式（邮箱、手机/微信）；但不要主动讨论或承诺具体薪资数字、期望薪资。
4. 回复语言跟随整个对话的主导语言（即对话中用户绝大多数消息所用的语言），不要只根据当前一条消息判断。中文对话中用户偶尔回 ok、thanks、good、nice 等单个英文单词或短语时，继续用中文回复；反之，英文对话中偶尔夹杂单个中文词也继续用英文。只有用户用另一门语言表达完整问题、或连续多条消息都换语言时，才切换回复语言。
...
// 入参硬约束：限制单条消息长度与消息条数，防止恶意大 prompt 放大单次调用成本。
...
// 校验前端传入的 messages：非空、条数与单条长度均在上限内。返回错误描述或 null。
function validateMessages(messages) {
  ...
  for (const m of messages) {
    if (!m || typeof m.content !== 'string') return 'invalid message: content must be a string';
    if (m.content.length > MAX_MESSAGE_LEN) return `message exceeds ${MAX_MESSAGE_LEN} chars`;
  }
  return null;
}
...
// 当前日期（北京时间 UTC+8）注入 system prompt，供回答"是否还在职"等时间相关问题。
// Worker 运行时 Date.now() 返回 UTC 毫秒，+8 小时换算北京时间后再取 UTC 年月日。
function currentDateContext() {
  const bj = new Date(Date.now() + 8 * 3600 * 1000);
  const wd = '日一二三四五六'[bj.getUTCDay()];
  return `\n\n## 当前时间（系统注入，供回答"是否还在职"等时间问题使用）\n今天是 ${bj.getUTCFullYear()}年${bj.getUTCMonth() + 1}月${bj.getUTCDate()}日 星期${wd}（北京时间）。`;
}
...
async function handleChat(request, env) {
  ...
  const vErr = validateMessages(body.messages);
  if (vErr) return json({ error: vErr }, 400);

  const messages = compactMessages([{ role: 'system', content: SYSTEM_PROMPT + currentDateContext() }, ...body.messages]);
  ...
  const requestBody = {
    ...
    stream: true,
    temperature: 0.5,
    // DeepSeek 输出上限 8192 token（≈1.2 万~1.6 万中文字符）。
    // 此前 1200 会把"十个面试题+详答"之类长回答以 finish_reason=length 截断，
    // 流随后正常 [DONE]，前端不报错只显示被腰斩的消息。8192 按实际用量计费，
    // 短回答零额外成本；zone WAF 限流 60/min + 入参 MAX_MESSAGE_LEN 兜底。
    max_tokens: 8192,
  };
...
async function handleSuggestions(request, env) {
  ...
  // 防止单条超长消息放大成本（suggestions 只取最近 6 条，条数本身已受限）
  if (userMessages.some((m) => m && typeof m.content === 'string' && m.content.length > MAX_MESSAGE_LEN)) {
    return json({ suggestions: [] });
  }
